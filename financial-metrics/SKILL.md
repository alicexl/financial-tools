...
description: Use when computing financial metrics from a Chinese A-share company's annual reports (PDFs in <cwd>/年报/<公司名>年报/ or any specified dir) — user mentions 杜邦分析, 三因子, 五因子, Z-score, Altman, ROE decomposition, 财务指标, 算下 XX 公司的杜邦/Z值, or wants to compute profitability/leverage/efficiency ratios from 年报. Triggers on company name + financial-analysis intent.
---
...
- 用户给公司名（年报已在 `<cwd>/年报/<公司名>年报/`，cwd = Claude Code 启动目录）或显式给 PDF 目录
...
1. **定位 PDF**: `<cwd>/年报/<公司名>年报/` 下按文件名 `\d{4}年` 提取年份，跳过摘要/英文版
...
4. **多年合并**: 每份年报 N 提供 (本期=N, 上期=N-1) 两份数据。**年份 N 优先采用 N 年报的"本期"（原始披露）**；仅当 N 年报缺失时才退用 N+1 年报的"上期"对照列（可能已被同一控制下企业合并重述，如海尔智家 2023 原始披露 2614 亿 vs 2024 年报"上期"重述为 2742 亿）
...
| 同一份数据多版本（追溯调整） | 自家年报本期优先；缺失时退用次年年报上期（已重述） |
